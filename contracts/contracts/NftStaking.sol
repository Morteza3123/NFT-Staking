...
contract NftStaking is Ownable, IERC721Receiver {
  ...
  function stake(uint256[] calldata tokenIds) external {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      require(vault[tokenId].isStaked == false, 'already staked');
      totalStaked += 1;
  ...
  function _unstakeMany(address account, uint256[] calldata tokenIds) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      require(staked.isStaked == true, "not an owner");
      totalStaked -= 1;
  ...
  function earningInfo(address account, uint256[] calldata tokenIds) external view returns (uint256[1] memory info) {
     ...
     for(uint i =0; i < tokenIds.length; i++) {
      ...
      require(staked.owner == account, "not an owner");
      uint256 stakedAt = staked.timestamp;
      rewardmatch += 100 ether * (block.timestamp - stakedAt) / 1 days;
      earned = rewardmatch/100;
     }
    if(earned > 0) {
      return [earned];
    }
